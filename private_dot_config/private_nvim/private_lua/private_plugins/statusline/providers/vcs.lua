...
function M.get_git_dir(path)
  ...
  return has_git_dir(path) or has_git_file(path) or -- Otherwise go up one level and make a recursive call
    (parent_path ~= path and M.get_git_dir(parent_path) or nil)
end
...
function M.get_git_branch_formatted()
  ...
  return M.get_git_branch()
end

function M.seperator()
  if diagnostic.has_diagnostics() and not M.check_git_workspace() then
    return i.slant.Left
  elseif not diagnostic.has_diagnostics() and M.check_git_workspace() then
    return i.slant.Left
  else
    return ""
  end
